/**
 * Maintains the mutable state used while building a code-path graph.
 *
 * The analyzer walks the AST once and relies on this state machine to split,
 * merge, and finalize control-flow segments for every syntactic construct that
...
import { ForkContext } from "./fork-context.js";
...
/**
 * Creates a pair of linked stores for tracking the final returned and thrown
 * segments of a code path. Each store exposes an `items` array and an `add()`
 * method. When segments are added to one store they are also added to the shared
 * `finalSegments` array, unless the segment already appears in the other store.
 * @param {CodePathSegment[]} finalSegments The shared array for all final segments.
 * @returns {{ returned: FinalSegmentsStore, thrown: FinalSegmentsStore }}
 */
function createFinalSegmentsStores(finalSegments) {
    const returned = { items: [] };
    const thrown = { items: [] };

    returned.add = function(segments) {
        for (let i = 0; i < segments.length; i += 1) {
            const segment = segments[i];
            returned.items.push(segment);
            if (!thrown.items.includes(segment)) {
                finalSegments.push(segment);
            }
        }
    };

    thrown.add = function(segments) {
        for (let i = 0; i < segments.length; i += 1) {
            const segment = segments[i];
            thrown.items.push(segment);
            if (!returned.items.includes(segment)) {
                finalSegments.push(segment);
            }
        }
    };
...
/**
 * State machine for code-path analysis.
 */
class CodePathState {
    /**
     * Creates a new analysis state container.
     * @param {IdGenerator} idGenerator Segment ID generator.
     * @param {Function} onLooped Callback invoked when a loop edge is created.
     */
    constructor(idGenerator, onLooped) {
        ...
        const { returned, thrown } = createFinalSegmentsStores(this.finalSegments);

        /**
         * The final segments of the code path which are `return`. These
         * segments are also contained in `finalSegments`.
         * @type {{ items: Array<CodePathSegment>, add: Function }}
         */
    ...
    /**
     * The current head segments of the active fork context.
     * @type {CodePathSegment[]}
     */
    get headSegments() {
    ...
    /**
     * The parent forking context used when creating new branches.
     * @type {ForkContext}
     */
    ...
    /**
     * Creates and stacks a new fork context.
     * @param {boolean} forkLeavingPath `true` when creating a fork for leaving a `finally` block.
    ...
    /**
     * Pops and merges the last fork context.
     * @returns {ForkContext} The removed context.
     */
    popForkContext() {
    ...
    /**
     * Creates a new branch from the last segment in the parent context.
     * @returns {void}
     */
    forkPath() {
    ...
    /**
     * Creates a bypass branch from the current head.
     * This is used for constructs such as `if` statements without an `else`.
     * @returns {void}
     */
    forkBypassPath() {
    ...
    popChainContext() {
        ...
        for (let i = context.choiceContextCount; i > 0; i -= 1) {
    ...
    popSwitchContext() {
        ...
        for (let i = 0; i < context.forkCount; i += 1) {
    ...
    makeSwitchCaseBody(isCaseBodyEmpty, isDefaultCase) {
        ...
        } else if (!isCaseBodyEmpty && context.foundEmptyDefault) {
            /*
    ...
    makeFinallyBlock() {
        ...
        for (let i = 0; i < forkContext.count; i += 1) {
